<template>
  <div class="container">
    <h1 class="title">Your Recipes Page</h1>
    <router-link v-if="!$root.store.username" to="/login" tag="button">You need to Login to view this</router-link>
    <Spinner :loading="loading" />
    <RecipePreviewList v-if="!loading" title="Your Recipes:" routeName="userRecipe" :recipes="recipes" :class="{
  ...
  async mounted() {
    try {
      ...
      if (!this.$root.store.username) {
        ...
        const response = await this.axios.get(`${this.$root.store.server_domain}/users/userRecipes`, { withCredentials: true });
